			...
			if (entity instanceof EntityPlayer) {
				...
				if(this.cooldown == 2) {
					...
					if (this == ModItems.antidote) {
						...
						if (HbmLivingProps.getOverdose(player) > 4000) {
							NBTTagCompound nbt = new NBTTagCompound();
							nbt.setString("type", "vomit");
							nbt.setString("mode", "normal");
							nbt.setInteger("count", 15);
		...
		if(this.cooldown == 0){
			if (stack.getItemDamage() != stack.getMaxDamage() && !VersatileConfig.hasPotionSickness(player)) {
				...
				if((this == ModItems.antidote)||(this == ModItems.painkiller)) {
					player.playSound("pills_blister", 1F, 1F);
				}
				if((this == ModItems.yadulin)||(this == ModItems.ibuprofen)) {
					player.playSound("pills_bottle", 1F, 1F);
				}
				System.out.println("sombody used a pill");
			}
		}
		return stack;
