...
public class TileEntityFurnaceIron extends TileEntityMachineBase implements IGUIProvider, IUpgradeInfoProvider {
	...
	public void updateEntity() {
		...
		if(!worldObj.isRemote) {
			...
			this.processingTime = (int) (baseTime * Math.pow(0.8, UpgradeManager.getLevel(UpgradeType.SPEED)));
	...
	public void provideInfo(UpgradeType type, int level, List<String> info, boolean extendedInfo) {
		...
		if(type == UpgradeType.SPEED) {
			info.add(EnumChatFormatting.GREEN + I18nUtil.resolveKey(this.KEY_DELAY, "-" + (level * 20) + "%"));
		}
	}
